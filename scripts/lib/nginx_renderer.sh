...
wm_nginx_install_candidate() {
  local candidate transaction_dir backup
  candidate="$1"; transaction_dir="$2"; backup="$transaction_dir/nginx.before.conf"
...
wm_nginx_apply_desired() {
  local config_file transaction_dir candidate
  config_file="$1"; transaction_dir="$2"; candidate="$transaction_dir/nginx.candidate.conf"
...
wm_nginx_apply_native_rotation_candidate() {
  local config_file transaction_dir old_path new_path candidate
  config_file="$1"; transaction_dir="$2"; old_path="$3"; new_path="$4"; candidate="$transaction_dir/nginx.native-rotation.conf"
