...
def build(args):
    ...
    if args.public_domain:
        payload["streamSettings"]["externalProxy"]=[{"dest":args.public_domain,"port":443,"remark":args.tag,"forceTls":"tls","sni":args.public_domain,"fingerprint":args.fingerprint}]
    return payload
...
def normalized(inbound):
    ...
    return {"tag":inbound.get("tag") or inbound.get("remark"),"listen":inbound.get("listen"),"port":int(inbound.get("port",-1)),"protocol":inbound.get("protocol"),"path":xhttp.get("path"),"host":xhttp.get("host"),"mode":xhttp.get("mode"),"clients":sorted((x.get("id"),x.get("email"),x.get("enable",True)) for x in clients if isinstance(x,dict))}
...
def main():
    ...
    make.add_argument("--tag",required=True); make.add_argument("--port",type=int,required=True); make.add_argument("--path",required=True); make.add_argument("--host",required=True); make.add_argument("--clients",required=True); make.add_argument("--public-domain",default=""); make.add_argument("--fingerprint",default="chrome"); make.add_argument("--output",required=True)
