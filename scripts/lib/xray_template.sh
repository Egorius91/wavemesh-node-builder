...
wm_xray_apply_managed_route() {
  local outbound_file="$1" inbound_tag="$2" outbound_tag="$3" rule_tag="$4" transaction_dir original candidate owns_transaction=0
  ...
  wm_xray_get_template "$original" || return 1; cp "$original" "$WM_STATE_DIR/backups/xray.$(date -u +%Y%m%dT%H%M%SZ).json"
