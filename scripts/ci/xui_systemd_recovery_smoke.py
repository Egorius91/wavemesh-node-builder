...
def run_inner(args):
    ...
    try:
        ...
        if args.replacement or args.maintenance_start or args.maintenance_recovery:
        ...
        if args.maintenance_start or args.maintenance_recovery:
            ...
            if args.maintenance_recovery:
                from xui_maintenance_recovery_smoke import run as recover
                recover(smoke, guard, lock, unit, OP, candidate_sha, rollback_sha,
                        manifest_sha, args.head, helper_sha, executable_sha)
                report = {'schema': 1, 'builder_commit': args.head, 'status': 'PASS', 'deployment': 'NONE',
                          'scope': 'DISPOSABLE_TERMINAL_MAINTENANCE_RECOVERY', 'stopped_and_failed_recovered': True,
                          'lost_results_reconciled': True, 'stale_attempts_denied': True,
                          'runtime_and_writers_denied': True, 'client_identities_unchanged': True,
...
def main():
    ...
    parser.add_argument('--maintenance-recovery', action='store_true')
    ...
    with tempfile.TemporaryDirectory(prefix='wm-real-recover-', dir='/run') as directory:
        ...
        try:
            ...
            result = subprocess.run(['nsenter', '-t', str(keeper.pid), '-n', sys.executable,
                                     ...
                                     *(['--maintenance-recovery'] if args.maintenance_recovery else [])],
