...
are generated locally and only their SHA-256 hashes are sent to SaaS.
"""
...
class NodeAgent:
    ...
    def run(self, once: bool = False) -> None:
        ...
        while not _STOP:
            ...
            sleep_for = max(1.0, self.config.heartbeat_seconds - elapsed)
            stop_aware_sleep(sleep_for)
    ...
    def collect_and_send_observation(self) -> None:
        ...
        observation_type = f"node.health.{state_hash[:16]}"
        idempotency_key = f"node-health-{self.config.node_id}-{state_hash[:32]}"
        self.api_json(
            "POST",
