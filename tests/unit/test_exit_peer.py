...
with tempfile.TemporaryDirectory() as name:
    ...
    subprocess.run([sys.executable,str(renderer),"--config",str(candidate),"--output",str(nginx)],check=True); rendered=nginx.read_text(); assert "allow 203.0.113.10;" in rendered and "deny all;" in rendered and "127.0.0.1:22001" in rendered; assert "00000000" not in rendered
